...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getGuestRequests() {
    const data = await prisma.guest_request
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'guest_request'));
  ...
  async function createGuestRequest() {
    ...
    const data = await prisma.guest_request.create({
      data: body,
    });
    return res.status(200).json(data);
  }
