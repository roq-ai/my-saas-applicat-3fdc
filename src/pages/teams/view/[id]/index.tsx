...
import { deleteGuestRequestById } from 'apiSdk/guest-requests';
...
function TeamViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<TeamInterface>(
    ...
    () =>
      getTeamById(id, {
        relations: ['user', 'guest_request', 'task', 'team_member'],
      }),
  );

  const guest_requestHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteGuestRequestById(id);
      await mutate();
    } catch (error) {
        ...
        {isLoading ? (
          ...
          <>
            ...
            <Box>
              ...
              {hasAccess('guest_request', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    <Text fontSize="lg" fontWeight="bold">
                      Guest Requests
                    </Text>
                    <NextLink passHref href={`/guest-requests/create?team_id=${data?.id}`}>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.guest_request?.map((record) => (
                          <Tr
                            cursor="pointer"
                            onClick={() => router.push(`/guest-requests/view/${record.id}`)}
                            key={record.id}
                          >
                            <Td>{record.status}</Td>
                            <Td>
                              {hasAccess('guest_request', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                                <NextLink
                                  onClick={(e) => e.stopPropagation()}
                                  passHref
                                  href={`/guest-requests/edit/${record.id}`}
                                >
                              ...
                              {hasAccess('guest_request', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                                <IconButton
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    guest_requestHandleDelete(record.id);
                                  }}
                                  colorScheme="red"
                                  variant="outline"
                                  aria-label="edit"
              ...
              {hasAccess('task', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/tasks/create?team_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
